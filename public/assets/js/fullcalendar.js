...
*/
    ...
    ------------------------------------------------------------------------------------------------------------------*/
                    ...
                    DayTableMixin.prototype.renderBgCellHtml = function (date, otherAttrs) {
                        ...
                        let data_name_input = document.getElementsByClassName('work_name');
                        let data_start_input = document.getElementsByClassName('starting_date');
                        let data_end_input = document.getElementsByClassName('ending_date');
                        var show_data_start = [];
                        var show_data_end = [];
                        for (let i = 0; i < data_start_input.length; i++){
                           if(date.format('DD-MM-YYYY') === data_start_input[i].innerHTML){
                               show_data_start.push(' '+data_name_input[i].innerHTML);
                               var class_start = 'btn btn-danger text-start d-block';
                           }
                            if(date.format('DD-MM-YYYY') === data_end_input[i].innerHTML){
                                show_data_end.push(' '+data_name_input[i].innerHTML);
                                var class_end = 'btn btn-success text-start d-block';
                            }
                        }
                        classes.unshift('fc-day', view.calendar.theme.getClass('widgetContent'));
                        return '<td id="data_date" value="'+date.format('YYYY-MM-DD')+'" class="' + classes.join(' ') + '"' +
                            (isDateValid ?
                            ...
                            (otherAttrs ?
                                ...
                                '') +
                            '>' +
                            '<div id="show_data_start" class="'+ class_start +'">'+ show_data_start +'</div>' +
                            '<div id="show_data_end" class="'+ class_end +'">'+ show_data_end +'</div>' +
                            '</td>';
                    };
                    /* Generic
    ------------------------------------------------------------------------------------------------------------------*/
    ...
    ------------------------------------------------------------------------------------------------------------------*/
                    ...
                    DayGrid.prototype.renderNumberCellHtml = function (date) {
                        ...
                        if (isDayNumberVisible) {
                            html += view.buildGotoAnchorHtml(date, { 'class': 'fc-day-number text-dark' }, date.format('D') // inner HTML
                            );
                        }
                        html += '</td>';
                        return html;
